...
export function AccountHnView({ quoteId }: { quoteId: number }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      qRef.current = q; dirtyRef.current = false; setReady(true); redraw();
    } catch (ex) { setErr(ex instanceof ApiError ? ex.message : "Lỗi tải"); }
  }, [quoteId, redraw]);
  useEffect(() => { load(); }, [load]);

  if (err) return <div className="err" style={{ margin: 24 }}>⚠ {err} <a className="btn btn-sm" href="#/list">Về danh sách</a></div>;
  ...
  const save = async (thenSubmit: boolean) => {
    ...
    try {
      ...
      await load();
    } catch (ex) { toast(ex instanceof ApiError ? ex.message : "Lỗi lưu phần HN", "error"); setSaving(false); }
  };
  ...
  return (
    <div className="account-hn-view ahn-card">
      <div className="ahn-head">
        <div className="ahn-head-titles"><h2 className="ahn-title">Phần Giá Hà Nội</h2>
        ...
        return (
          <div key={hs.sheetId ?? si} className="extra-cat-group" style={{ marginTop: 10 }}>
            <div className="extra-cat-grouphead">
              ...
              {editable && <button type="button" className="btn btn-sm" onClick={() => { const cur = hs.hnTables[hs._activeHn || 0]; hs.hnTables.push(newHnTable(cur?.templateId)); hs._activeHn = hs.hnTables.length - 1; mark(); redraw(); }}>+ Thêm sheet</button>}
              <span className="muted" style={{ fontSize: 11.5 }}>{hs.hnTables.length} sheet</span>
            </div>
            <div className="sheet-tabs extra-sheet-tabs">
              {hs.hnTables.map((tt, ti) => (
                <div key={tt._k ?? ti} className={`sheet-tab ${ti === active ? "active" : ""}`} onClick={() => { hs._activeHn = ti; redraw(); }}>
                  <span>{tt.name || `Bảng ${ti + 1}`}</span>
                  {editable && hs.hnTables.length > 1 && <span className="rm-tab" title="Xoá sheet này" onClick={(e) => { e.stopPropagation(); hs.hnTables.splice(ti, 1); let a = hs._activeHn || 0; if (a > ti) a--; if (a >= hs.hnTables.length) a = hs.hnTables.length - 1; if (a < 0) a = 0; hs._activeHn = a; mark(); redraw(); }}>✕</span>}
                </div>
              ))}
            </div>
            <div className="extra-table">
              <div className="extra-table-head" style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", margin: "8px 0" }}>
                <span className="extra-cat-badge cat-hanoi">Báo Giá Hà Nội</span>
                <input className="extra-name" defaultValue={t.name || ""} placeholder="Tên sheet (tuỳ chọn)" disabled={!editable} style={{ minWidth: 180 }} onInput={(e) => { t.name = (e.target as HTMLInputElement).value; mark(); redraw(); }} />
