...
export const api = {
  ...
  // `datLaiMatKhau` phân biệt hai công dụng của CÙNG một đường: tài khoản CHƯA kích hoạt = nhận lời
  // mời lần đầu (phải khai hồ sơ), tài khoản ĐÃ kích hoạt = quên mật khẩu (chỉ đổi mật khẩu).
  getInvite: (token: string) => req<{ email: string; displayName?: string; datLaiMatKhau?: boolean }>(`/auth/invite/${encodeURIComponent(token)}`),
