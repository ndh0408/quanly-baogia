...
import { _datLai } from "../lib/phienBan";
...
beforeEach(() => { _datLai(); h.taiBanMoi.mockClear(); h.luuRoiBao.mockClear(); (window as Window & { __editorDirty?: boolean }).__editorDirty = false; });
...
describe("dải thông báo bản mới", () => {
  ...
  it("có bản mới, không dở gì → 'Tải bản mới' tải ngay", () => {
    act(() => _datLai(BAN_MOI));
    ve(<ThongBaoBanMoi />);
  ...
  it("còn thay đổi CHƯA LƯU → câu nhắc lưu trước; bấm Tải bản mới thì HỎI (Lưu rồi tải / Tải luôn / Hủy), không tải ngay", () => {
    ...
    expect(nut("Tải luôn (bỏ thay đổi)")).toBeDefined();
    act(() => nut("Hủy")!.click());
