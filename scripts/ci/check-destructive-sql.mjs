...
//
// ── CỐ Ý KHÔNG BẮT ───────────────────────────────────────────────────────────
//   • `DROP TYPE` / `DROP CONSTRAINT` / `DROP INDEX`: không xoá hàng nào. Prisma sinh cặp
//     "tạo enum mới → DROP TYPE cũ" cho MỌI lần đổi enum; bắt nó thì gác thành tiếng ồn và người
//     ta sẽ thêm allowlist theo phản xạ.
...
export function quetMigrations(root = ROOT) {
  ...
  for (const ten of readdirSync(goc).sort()) {
    ...
    const file = join(duong, "migration.sql");
    let sql;
    try { sql = readFileSync(file, "utf8"); } catch { continue; } // thư mục không có migration.sql
