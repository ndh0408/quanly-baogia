...
| Metrics | Prometheus (`prom-client` 15.1) | **KEEP** | — | 31 metric riêng + bộ mặc định (đếm: `grep -coE 'name: "[a-z_]+"' src/observability.ts`). `/metrics` gác bằng Bearer, **404 ở production nếu thiếu token**. | — |
