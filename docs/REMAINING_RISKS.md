...
| 5 · Observability | **xong** (2026-08-27) | 20 rule cảnh báo + 35 bài `promtool test rules` · ngăn xếp Loki+Promtail+Grafana chạy được ở `infra/observability/` (opt-in) · log đủ 7 trường §27 |
...
- **Operations → dashboards, alerts**: ✅ 20 rule + 35 bài kiểm logic; bảng điều khiển
...
`infra/prometheus/alerts.yaml` — 20 quy tắc, 7 nhóm, mỗi cái bám một chế độ hỏng có
...
`infra/prometheus/alerts.test.yaml` — 35 bài `promtool test rules`, gồm cả vế **chống
