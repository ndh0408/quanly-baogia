...
| Lệnh | Kết quả |
|---|---|
| `npx vitest run` (backend) | **183 tệp** — đo 2026-09-07. Đợt này thêm `tests/projectref-sheet-discount.test.js` (Discount ở mức sheet) và `tests/projectcode-parity.test.js` (mã dự án: bản server `src/quoteCode.ts` và bản web `shared/quote-math.ts` không được lệch); chạy riêng hai bài: **12 bài xanh**, không cần hạ tầng |
