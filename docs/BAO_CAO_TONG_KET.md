...
| Lệnh | Kết quả |
|---|---|
| `npx vitest run` (backend) | đã đo lúc 2026-09-07: **183 tệp**. Đợt đó thêm `tests/projectref-sheet-discount.test.js` (Discount ở mức sheet) và `tests/projectcode-parity.test.js` (mã dự án: bản server `src/quoteCode.ts` và bản web `shared/quote-math.ts` không được lệch); chạy riêng hai bài đó lúc ấy: **12 bài xanh**, không cần hạ tầng. (Đo lại: `git ls-files 'tests/*.test.js' \| wc -l`.) |
...
| `npx vitest run` trong `web/` | đã đo lúc 2026-08-28: **22 tệp · 293 bài xanh**; đã đo lại 2026-09-09: **23 tệp · 307 bài xanh**. Mặc định vẫn là environment `node` (`web/vite.config.ts` không khai khối `test`); đúng một tệp bật jsdom bằng docblock `@vitest-environment`, xem [development/TESTING.md](development/TESTING.md) |
