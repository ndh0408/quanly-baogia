...
```bash
grep -oE 'name: "[a-z_]+"' src/observability.ts | wc -l   # 24 metric ứng dụng
grep -c '^      - alert:' infra/prometheus/alerts.yaml    # 20 quy tắc cảnh báo
```
...
> 20 quy tắc trong `infra/prometheus/alerts.yaml` **được Prometheus đánh giá thật** và **chuyển sang
